      ...
      <div className="proj-body">
        <div className="proj-tags">
          {project.tags.map((tag) => (
            <span className={tag.tone ? `tag ${tag.tone}` : "tag"} key={tag.label}>
              {tag.label}
            </span>
          ))}
        </div>
        ...
        {project.impact && <div className="proj-impact-text">{project.impact}</div>}
        {project.confidential && (
          <p className="proj-confidential">
            Case apresentado de forma resumida, respeitando confidencialidade e dados
            sensíveis.
          </p>
        )}
        <div className="proj-footer">
          {project.confidential ? (
            ...
            project.cta && (
              <a href={project.cta.href} className="proj-cta">
                <Eye size={12} />
                {project.cta.label}
              </a>
            )
